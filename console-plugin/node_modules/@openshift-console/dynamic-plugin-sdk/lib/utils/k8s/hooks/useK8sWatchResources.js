...
export const useK8sWatchResources = (initResources) => {
    ...
    const k8sModelsRef = useRef(ImmutableMap());
    ...
    const reduxIDs = useMemo(() => modelsLoaded
    ...
    const resourceK8sSelectorCreator = useMemo(() => createSelectorCreator({
        memoize: lruMemoize,
        memoizeOptions: {
            equalityCheck: (oldK8s, newK8s) => Object.keys(reduxIDs || {})
                .filter((k) => !reduxIDs[k].noModel)
                .every((k) => oldK8s.get(reduxIDs[k].id) === newK8s.get(reduxIDs[k].id)),
        },
    }), [reduxIDs]);
    const resourceK8sSelector = useMemo(() => resourceK8sSelectorCreator((state) => state.k8s, (k8s) => k8s), [resourceK8sSelectorCreator]);
    const resourceK8s = useSelector(resourceK8sSelector);
    const results = useMemo(() => Object.keys(resources).reduce((acc, key) => {
